...
bs = 16
num_epochs = 15
# We define if we are using the segmentation feature
seg = True
...
def train_model(model, dataloaders, criterion, optimizer, num_epochs, model_name):
    ...
    with tqdm(total=num_epochs) as pbar:
        ...
        for epoch in range(num_epochs):
            ...
            for phase in ['train', 'val']:
                ...
                print('{} Loss: {:.4f} F-score: {:.4f}'.format(phase, epoch_loss, epoch_acc))
    ...
    print('Best validation F-score: {:4f}'.format(best_acc))
...
def initialize_model(num_classes, use_pretrained=True, segmentation_channel=False):
    ...
    if segmentation_channel:
        ...
        model_ft.features[0] = nn.Conv2d(4, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
        mean = torch.mean(sd['features.conv0.weight'], dim = 1, keepdim = True)
        sd['features.conv0.weight'] = torch.cat((sd['features.conv0.weight'], mean), dim=1)
...
model_ft, input_size = initialize_model(num_classes, use_pretrained=False, segmentation_channel = seg)
...
# Train and evaluate
model_ft, hist = train_model(model_ft, dataloaders_dict, criterion, optimizer_ft, num_epochs=num_epochs, model_name='PreTrainedSeg.pt')
